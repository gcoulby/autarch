...
function ComboboxTrigger({ className, children, ...props }: ComboboxPrimitive.Trigger.Props) {
  return (
    <ComboboxPrimitive.Trigger data-slot="combobox-trigger" className={cn("[&_svg:not([class*='size-'])]:size-4", className)} {...props}>
      {children}
      <ChevronDownIcon data-slot="combobox-trigger-icon" className="size-4 text-muted-foreground pointer-events-none" />
...
function ComboboxList({ className, ...props }: ComboboxPrimitive.List.Props) {
  return (
    <ComboboxPrimitive.List
      ...
      className={cn(
        'p-1 data-empty:p-0 max-h-[min(calc(--spacing(96)---spacing(9)),calc(var(--available-height)---spacing(9)))] overflow-y-auto scroll-py-1',
        className,
      )}
      {...props}
    />
  ...
  return (
    ...
    >
      ...
      <ComboboxPrimitive.ItemIndicator
        ...
        render={<span className="right-2 absolute flex justify-center items-center size-4 pointer-events-none" />}
      >
        <CheckIcon className="size-4 pointer-coarse:size-5 pointer-events-none" />
      </ComboboxPrimitive.ItemIndicator>
    </ComboboxPrimitive.Item>
  )
}
...
function ComboboxLabel({ className, ...props }: ComboboxPrimitive.GroupLabel.Props) {
  return (
    <ComboboxPrimitive.GroupLabel
      ...
      className={cn('px-2 pointer-coarse:px-3 py-1.5 pointer-coarse:py-2 text-muted-foreground text-xs pointer-coarse:text-sm', className)}
      {...props}
    />
  )
}
...
function ComboboxEmpty({ className, ...props }: ComboboxPrimitive.Empty.Props) {
  return (
    <ComboboxPrimitive.Empty
      ...
      className={cn('hidden group-data-empty/combobox-content:flex justify-center py-2 w-full text-muted-foreground text-sm text-center', className)}
      {...props}
    />
  )
}
...
function ComboboxSeparator({ className, ...props }: ComboboxPrimitive.Separator.Props) {
  return <ComboboxPrimitive.Separator data-slot="combobox-separator" className={cn('-mx-1 my-1 bg-border h-px', className)} {...props} />
}
...
function ComboboxChips({ className, ...props }: React.ComponentPropsWithRef<typeof ComboboxPrimitive.Chips> & ComboboxPrimitive.Chips.Props) {
  return (
    <ComboboxPrimitive.Chips
      ...
      className={cn(
        'flex flex-wrap items-center gap-1.5 bg-transparent dark:bg-input/30 bg-clip-padding shadow-xs px-2.5 has-data-[slot=combobox-chip]:px-1.5 py-1.5 border border-input has-aria-invalid:border-destructive focus-within:border-ring dark:has-aria-invalid:border-destructive/50 rounded-md has-aria-invalid:ring-[3px] has-aria-invalid:ring-destructive/20 focus-within:ring-[3px] focus-within:ring-ring/50 dark:has-aria-invalid:ring-destructive/40 min-h-9 text-sm transition-[color,box-shadow]',
        className,
      )}
      {...props}
    />
...
function ComboboxChipsInput({ className, children, ...props }: ComboboxPrimitive.Input.Props) {
  return <ComboboxPrimitive.Input data-slot="combobox-chip-input" className={cn('flex-1 outline-none min-w-16', className)} {...props} />
}
